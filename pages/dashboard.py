...
total_fc = future_df["pred_qty"].sum()
total_act = hist_df["qty_actual"].sum()
monthly = hist_df.groupby("periode")["qty_actual"].sum()
avg_sales = monthly.mean() if not monthly.empty else 0
...
df_chart = df_chart[
    (df_chart["qty_actual"] > 0) | (df_chart["pred_qty"] > 0)
]
...
if not future_df.empty:
    ...
    df_future_agg = pd.DataFrame(columns=["cabang", "sku", "forecast_total"])
...
if df_mape is not None:
    ...
    if "mape_sku" in df_stock.columns:
        df_stock["mape_used"] = df_stock["mape_sku"].fillna(base_mape)
    else:
...
cond_excess = (
    df_stock["last_stock"].notna()
    & (df_stock["last_stock"] > df_stock["target_stock"])
)
...
n_short = int((df_kpi["Status"] == "Risiko Kekurangan").sum())
n_excess = int((df_kpi["Status"] == "Potensi Kelebihan").sum())
avg_cov = df_kpi["coverage_month"].mean()
if pd.isna(avg_cov):
    avg_cov = 0.0
